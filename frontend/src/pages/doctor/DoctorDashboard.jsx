        ...
        <nav className="flex-1 space-y-3">
          {navItems.map(item => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) => 
                `w-full flex items-center gap-4 px-6 py-4.5 rounded-[1.5rem] transition-all duration-500 font-black text-xs uppercase tracking-widest ${
                  isPathActive(item.path) ? 'bg-[#427CFF] text-white shadow-2xl shadow-blue-100 scale-[1.02]' : 'text-gray-400 hover:text-[#111] hover:bg-gray-50'
                }`
              }
            >
      ...
      <main className="flex-1 ml-80 p-12 overflow-y-auto">
        <header className="mb-14 flex justify-between items-end">
           <div>
              ...
              <h1 className="text-[#111] text-6xl font-black tracking-tighter leading-none italic">
                {getPageTitle()}.
              </h1>
           </div>
